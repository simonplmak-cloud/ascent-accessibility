  ...
  return (
    ...
    >
      <input
        ...
        className="w-full border-b border-terminal-border bg-terminal-surface px-4 py-3 font-sans text-sm text-terminal-fg placeholder:text-terminal-muted focus:outline-none focus:ring-2 focus:ring-terminal-fg"
      />
      <ul id="palette-list" role="listbox" aria-label={t("commands")} className="max-h-80 overflow-y-auto p-1">
        {ranked.length === 0 && (
          <li role="option" aria-selected="false" aria-disabled="true" className="px-3 py-2 font-sans text-sm text-terminal-muted">{t("noCommands")}</li>
        )}
        {ranked.map((cmd, i) => (
