...
export function PracticeCheck({
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/training/lessons/${lessonId}/check?locale=${locale}`, {
        method: "POST",
  ...
  return (
    <div className="mt-6 rounded border border-terminal-border bg-terminal-surface/40 p-4">
      <h2 className="font-sans text-sm font-semibold text-terminal-fg">{t("practiceCheck")}</h2>
          ...
          <p className="font-sans text-sm">
            ...
            {!result.correct && (
              <span className="text-terminal-muted">
                {" "}
                — {t("correctAnswer", { answer: question.options[result.correctIndex] ?? "" })}
              </span>
            )}
          </p>
