...
import { LEGAL_LINKS, SITE_SECTIONS } from "@/lib/site/navigation";
              ...
              <ul className="mt-3 m-0 list-none space-y-6 p-0">
                {section.children.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="inline-flex min-h-11 items-center font-sans text-sm text-terminal-muted underline-offset-4 hover:text-terminal-fg hover:underline"
                    >
                      {t(link.label)}
                    </Link>
                  </li>
          ...
          <nav aria-label="Legal" className="flex flex-wrap gap-x-5 gap-y-2">
            {LEGAL_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="inline-flex min-h-11 items-center font-sans text-xs text-terminal-muted underline-offset-4 hover:text-terminal-fg hover:underline"
              >
                {t(link.label)}
              </Link>
            ))}
