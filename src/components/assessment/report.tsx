...
import { priorityFindings } from "@/lib/report/report-priority";
...
export function Report({ result }: { result: AssessmentResult }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 font-sans text-xs text-terminal-muted">{tStorage("retentionNotice")}</p>

      {result.detectedLanguages && result.detectedLanguages.length > 0 && (
        <p className="mt-2 font-sans text-xs text-terminal-muted">
          {t("detectedLanguages")}: {result.detectedLanguages.join(" · ")}
        </p>
      )}
