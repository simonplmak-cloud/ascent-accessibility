...
// --- Provisioned-AI top-up (one-time credit purchase) ---

export interface AiTopupStore {
  credit(userId: string, amountCents: number, sessionId: string): Promise<boolean>;
}

// Returns the credited account id when this session is a new AI top-up that was
// successfully credited; null otherwise (not a top-up, replay, or invalid).
export async function handleAiTopup(
  session: Stripe.Checkout.Session,
  store: AiTopupStore,
): Promise<string | null> {
  if (session.metadata?.kind !== "ai-topup") return null;
  const userId = session.client_reference_id ?? session.metadata?.userId ?? null;
  const amountCents = session.amount_total ?? 0;
  if (!userId || amountCents <= 0) return null;
  const credited = await store.credit(userId, amountCents, session.id);
  return credited ? userId : null;
}
