...
export function createApiKeyService(store: ApiKeyStore): ApiKeyService {
  return {
    ...
    async authenticate(rawKey) {
      ...
      // A key without an owner cannot be used — programmatic access is for
      // verified accounts only. Rejecting here prevents owner-less assessments.
      if (!stored.userId) return { ok: false, reason: "invalid" };
