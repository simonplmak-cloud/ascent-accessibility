...
export async function resolveOwnerAi(ownerId: string): Promise<OwnerAiConfig | null> {
  const rows = await query<{
    ...
    "SELECT aiApiKey, aiProvider, aiBaseUrl, aiVisionModel, aiAudioModel, aiKeyKind FROM user WHERE id = type::record($id) LIMIT 1",
    { id: ownerId },
  );
  const row = rows[0];
...
// Lightweight "does this account have any saved AI key" check (no decrypt).
export async function hasOwnerAiKey(ownerId: string): Promise<boolean> {
  const rows = await query<{ c: number }>(
    "SELECT count() AS c FROM user WHERE id = type::record($id) AND aiApiKey IS NOT NONE LIMIT 1",
    { id: ownerId },
  );
  return (rows[0]?.c ?? 0) > 0;
}
