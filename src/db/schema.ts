...
DEFINE FIELD sitemapUrls ON assessment TYPE option<string>;
DEFINE FIELD pages ON assessment TYPE option<string>;
DEFINE FIELD bytes ON assessment TYPE int DEFAULT 0;
...
DEFINE FIELD detectedLanguages ON assessment TYPE option<string> DEFAULT NONE;
...
DEFINE INDEX audit_log_created_idx ON audit_log FIELDS createdAt;`,

  `DEFINE TABLE evidence SCHEMAFULL;
DEFINE FIELD assessmentId ON evidence TYPE string;
DEFINE FIELD ownerId ON evidence TYPE option<string> DEFAULT NONE;
...
DEFINE FIELD html ON evidence TYPE option<string>;
DEFINE FIELD bytes ON evidence TYPE int DEFAULT 0;
DEFINE FIELD compacted ON evidence TYPE bool DEFAULT false;
...
DEFINE INDEX evidence_owner_idx ON evidence FIELDS ownerId;`,
...
DEFINE FIELD aiKeyKind ON user TYPE option<string> DEFAULT NONE;
DEFINE FIELD aiBalanceCents ON user TYPE option<int> DEFAULT 0;
DEFINE FIELD openrouterKeyHash ON user TYPE option<string>;
...
DEFINE INDEX user_oauth_link_user_idx ON user_oauth_link FIELDS user;`,
  ...
  `DEFINE TABLE stripe_topup SCHEMAFULL PERMISSIONS
  FOR select, create, update, delete NONE;
DEFINE FIELD sessionId ON stripe_topup TYPE string;
DEFINE FIELD userId ON stripe_topup TYPE string;
DEFINE FIELD amountCents ON stripe_topup TYPE int;
DEFINE FIELD createdAt ON stripe_topup TYPE datetime DEFAULT time::now();
DEFINE INDEX stripe_topup_session_idx ON stripe_topup FIELDS sessionId UNIQUE;`,
