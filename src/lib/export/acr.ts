...
/** VPAT conformance level for a per-criterion row. */
export function vpatLevelOf(row: ConformanceRow): VpatLevel {
  switch (row.result) {
    case "Passed":
      return "supports";
    case "Failed":
...
const VPAT_EN: Record<VpatLevel, string> = {
  ...
  "needs-human-review": "Not evaluated — needs human review",
...
export function buildAcrHtml(input: AcrInput): string {
  ...
  <td>${esc(row.title)}</td>
  <td>${esc(row.level)}</td>
  <td>${esc(VPAT_EN[vpatLevelOf(row)])}</td>
