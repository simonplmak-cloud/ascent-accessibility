...
import { assessStorageQuota, STORAGE_QUOTA_BYTES_PER_USER, usedBytesByOwner } from "@/lib/storage-quota";
...
export async function POST(req: Request) {
  ...
  // Per-user storage quota (hard reject + pre-estimate) before queueing.
  if (ownerId) {
    const usedBytes = await usedBytesByOwner(ownerId);
    const decision = assessStorageQuota(usedBytes, STORAGE_QUOTA_BYTES_PER_USER, crawlScope.pageCap);
    if (!decision.allowed) {
      return NextResponse.json(
        {
