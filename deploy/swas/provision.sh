...
#   BYOK_ENCRYPTION_SECRET (required — must match Vercel; encrypts/decrypts
#                        per-account BYOK AI keys)
...
: "${BYOK_ENCRYPTION_SECRET:?BYOK_ENCRYPTION_SECRET is required (must match Vercel — encrypts/decrypts per-account BYOK AI keys)}"
...
umask 077
{
  ...
  printf 'BYOK_ENCRYPTION_SECRET=%s\n' "$BYOK_ENCRYPTION_SECRET"
