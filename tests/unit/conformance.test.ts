...
describe("computeConformance (machine verdict)", () => {
  ...
  it("marks content-absent SCs as NotPresent (exhaustive), partial as Unresolved", () => {
    const result = computeConformance(scsForStandard("2.2", "A"), [], new Set(), new Set(), EMPTY_FEATURES);
    expect(result.rows.find((r) => r.num === "1.2.1")?.result).toBe("NotPresent");
    expect(result.rows.find((r) => r.num === "1.1.1")?.result).toBe("Unresolved");
  });

  it("marks applicable-but-untested SCs as Unresolved", () => {
    const result = computeConformance(scsForStandard("2.2", "A"), [], new Set(), new Set(), FEATURES);
...
describe("finalizeConformance (final verdict)", () => {
  ...
  it("claims a level when every applicable SC is satisfied", () => {
    const scs = scsForStandard("2.2", "AA");
    const machine = {
      total: scs.length,
      passed: scs.length,
      failed: 0,
      ...
      unresolved: 0,
      coverage: 100,
      rows: scs.map((sc) => ({ num: sc.num, title: sc.title, level: sc.level, result: "Passed" as const })),
    };
