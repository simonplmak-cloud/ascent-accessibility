...
describe("DEFAULT_AI_CONFIGS", () => {
  ...
  it("non-judgeable vision SCs carry no evidence lists", () => {
    for (const c of Object.values(DEFAULT_AI_CONFIGS)) {
      if (c.modality === "vision" && !c.judgeable) {
        expect(c.whatToLookFor).toHaveLength(0);
        expect(c.passRequires).toHaveLength(0);
        expect(c.failRequires).toHaveLength(0);
      }
    }
  });
