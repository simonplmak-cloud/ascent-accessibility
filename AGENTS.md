...
Web accessibility assessment platform for Ascent Partners. A visitor submits a domain + standard (WCAG 2.2 AA default); the system crawls the site (sitemap-first), runs the Ascent Access accessibility engine in a self-hosted Chromium browser, and returns a score + findings + recommendations — exportable as PDF/CSV, with a live scan log, a Stripe subscription, and a donation page.
...
Unit tests (`tests/unit/*.test.ts`) are pure dependency-injected (in-memory fakes, no DB/browser/network); follow `AssessmentDeps`. E2E (`tests/e2e/*.spec.ts`, Playwright) needs a running app; not in `pnpm test`.
