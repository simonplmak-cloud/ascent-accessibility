...
Key modules: `src/lib/assessment` (orchestration, DI-friendly), `src/lib/crawler`, `src/lib/scanner`, `src/lib/scoring`, `src/lib/recommendations`, `src/lib/export`, `src/lib/engine` (clean-room in-house WCAG rule engine — the sole scanner), `src/lib/ai-review` (Qwen-VL needs-review triage with fail-safe), `src/lib/comparison` (consolidate, site-audit), `src/db/repository`, `src/server/*` (SSRF, rate-limit, api-keys, stripe, scanner-factory, auth).
